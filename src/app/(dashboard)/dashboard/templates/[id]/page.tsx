...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  await connectDB()
  const t = await Template.findById(id).select('title description').lean()
  if (!t) return { title: 'Template not found' }
  return { title: `${(t as any).title} — SlideVantage` }
}

export default async function DashboardTemplateDetailPage({ params }: Props) {
  const { id } = await params
  await connectDB()
